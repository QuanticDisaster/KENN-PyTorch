...
import sys
#sys.path.append(r'D:\KENN\KENN-PyTorch-master')
sys.path.append(r'..')
...
class Net(torch.nn.Module):
    def __init__(self, NUM_CLASSES, NUM_FEATURES, dim_model, k=16):
        ...
        #TODO : initially the F.log_softmax is given with the argument (..., dim=-1), à voir si ça ne bloque pas un truc ici...
        self.kenn_layer = relational_parser('knowledge.txt')#, activation=F.log_softmax)

    def forward(self, data):
        x, pos, batch = data.x, data.pos, data.batch
                
        x = torch.ones( size= (batch.shape[0], 1), device=pos.device) if x is None else x
